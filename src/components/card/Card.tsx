...
export const Card: React.FC<{ card: CardItem; week?: boolean; hidePrice?: boolean; restPage?: boolean; dishPage?: boolean; }> = ({ card, week, hidePrice, restPage, dishPage }) => {
  if ("name" in card && "chef" in card) return (
      <>
  ...
  if ("fullName" in card && "newChef" in card) return (
      <>
...
export const DisplayStars:React.FC<{rate:number}> = ({rate}) => {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < rate) {
      stars.push( <div key={`full-star ${i}`} className="star"> <FullStar /> </div> );
    } else {
      stars.push( <div key={`empty-start ${i}`} className="star"> <EmptyStar /> </div> );
    }
  }
